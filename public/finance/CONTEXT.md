...
>
> 🔥 **Інцидент 2026-07-17:** обидва додатки (фінанси і сліст) разом показали
> «0 операцій» — правила бази були з тестового режиму і мали вшиту дату
> закінчення (`".read": "now < …"`); коли вона минула, база відрубала ВСІ
> читання. Дані не постраждали. Лік: Console → Realtime Database → Rules →
> `{ "rules": { ".read": true, ".write": true } }` → Publish. Якщо таке
> повториться — першим ділом перевіряти Rules на дату-обмеження.
...
- ⭐ **Резервні копії бази (домовлено зробити).** База публічна на читання і
  запис, без бекапів — будь-хто з URL теоретично може стерти дані, і відкату
  нема. Варіанти (обрати при нагоді):
  1. **Cloudflare Worker з cron** (інфра вже є — `workers/`): раз на добу
     тягне `…firebasedatabase.app/finance.json` (+ `shopping.json`) і кладе
     знімок у KV/R2. Безкоштовно, повністю автоматично. — виглядає найкращим.
  2. GitHub Action по крону, що зберігає знімок як artifact (не комітити в
     публічне репо — це фінансові дані!).
  3. Blaze-план Firebase має вбудовані автобекапи (платно).
  Разом з цим варто колись закрити базу нормальною авторизацією (Firebase
  Auth анонімний вхід + правила по uid) — тоді і бекапи, і захист від чужих.
