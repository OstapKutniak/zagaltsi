...
// без зміни сцени). Вдягнене одразу відображається на персонажі (setEquipment).
...
export function buildInvPanel(
  scene: Phaser.Scene, offX: number, offY: number,
  getChar: () => CutoutCharacter | null,
): InvPanel {
  let equip: EquipState = loadEquip();
  let ui: Phaser.GameObjects.GameObject[] = [];
  applyEquipTo(getChar(), equip);

  const toggle = (item: InvItem): void => {
    equip[item.slot] = equip[item.slot] === item.id ? null : item.id;
    saveEquip(equip);
    applyEquipTo(getChar(), equip);
    draw();
  };
  ...
  function draw(): void {
    for (const o of ui) o.destroy();
    ui = [];
    const add = <T extends Phaser.GameObjects.GameObject>(o: T): T => { ui.push(o); return o; };
    ...
    for (let i = 0; i < SLOT_ORDER.length; i++) {
      ...
      const g = add(scene.add.graphics().setScrollFactor(0).setDepth(20));
      ...
      if (item) add(scene.add.image(x, y, itemIconTexture(scene, item)).setScrollFactor(0).setDepth(21).setScale(0.8));
      add(scene.add.text(x, y + SLOT_SIZE / 2 + 4, SLOT_LABELS[slot], {
      ...
      if (item) {
        const zone = add(scene.add.rectangle(x, y, SLOT_SIZE, SLOT_SIZE, 0, 0)
          .setScrollFactor(0).setDepth(22).setInteractive({ useHandCursor: true }));
        zone.on('pointerup', () => toggle(item));
      }
    }
    ...
    add(scene.add.text(LOGICAL_W / 2 + offX, stripY - CELL / 2 - 6, 'Речі', {
    ...
    const hint = scene.add.text(LOGICAL_W / 2 + offX, stripY + CELL / 2 + 6, '', {
    ...
    CATALOG.forEach((item, i) => {
      ...
      add(scene.add.image(x, stripY, itemIconTexture(scene, item))
        .setScrollFactor(0).setDepth(21).setScale(0.66).setAlpha(equipped ? 1 : 0.75));

      const zone = add(scene.add.rectangle(x, stripY, CELL, CELL, 0, 0)
        .setScrollFactor(0).setDepth(22).setInteractive({ useHandCursor: true }));
