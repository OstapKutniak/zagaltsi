...
interface EdLevel {
  ...
  enemySpawns: string[]; spawnCfg?: Record<string, unknown>; neutralSpawns: string[]; spawn: { x: number; y: number };
...
export async function stageLevelById(levelId: string | undefined | null): Promise<boolean> {
  ...
  const doc: Record<string, unknown> = {
    ...
    enemySpawns: lv.enemySpawns ?? [], spawnCfg: lv.spawnCfg ?? {}, neutralSpawns: lv.neutralSpawns ?? [],
