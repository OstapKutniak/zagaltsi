...
import { type Atmosphere, type WeatherPhase, type LayerKey, type FogLayer, evalTod } from './atmosphere';
import { buildAtmospherePanel } from './atmospherePanel';
...
export function initLevelEditor(prefix: string): void {
  ...
  function openLibMenu(a: typeof state.assets[0], clientX: number, clientY: number): void {
    ...
    if (hasAiBgRemoval()) {
      btn('🪄 ШІ виріз фону', async () => {
        setStatus('ШІ вирізає фон…');
        try {
          const url = await removeBackgroundAI(a.url);
          a.url = url; save(); refreshAssets(); draw(); setStatus('Фон вирізано (ШІ)');
        } catch (e) { setStatus('ШІ виріз фону не вдався: ' + ((e as Error).message || '')); }
      });
    }
  ...
  // Панель атмосфери — спільний модуль (той самий UI у редакторах локацій/меню).
  function renderAtmPanel(): void {
    if (!atmPanel) return;
    buildAtmospherePanel(atmPanel, () => {
      const lv = level();
