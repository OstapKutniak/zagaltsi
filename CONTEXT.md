...
### Паралакс
- Зберігається в `level.parallax: { bg: number; sky: number }` (0..1)
- 0 = рухається разом з картою, 1 = нерухоме (найдальше)
- Defaults: `{ bg: 0.5, sky: 0.8 }`
- UI: кнопка-перемикач «Фон/Небо» + слайдер «Дальність» у `#lv-levelToolbar`
- В грі: `setScrollFactor(1 - dist, 1)` на Phaser спрайтах bg/sky
