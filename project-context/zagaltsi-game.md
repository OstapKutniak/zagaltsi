...
- **Гра:** `floorFrac = vig.top`. Depth вінь'єтки **4999** (раніше 5.95): одразу під `foreground`(5000), але НАД персонажем (depth=`fy`≈340..520) і картою. Multiply затемнює все позаду (карта+ассети+персонаж), передній план лишається поверх. Фон вище горизонту нейтральний (текстура біла над `top`).
- **Редактор:** `drawVignette()` викликається МІЖ шаром `map`(+decor/collider/...) і `foreground` (а не останньою) — у todEnabled-циклі перед `lk==='foreground'`, у non-tod — між проходами non-foreground / foreground. Рендериться всередині кадру 1280×576 → 1:1 з грою.
- **Відкладено:** справжнє маскування (виключити фон, що заходить у смугу) — потрібен render-texture pass; поки фон у смузі мінімальний бо об'єкти стоять базою на горизонті. Повзунок «Верх» — тимчасове рішення, повернутись до авто-прив'язки.
