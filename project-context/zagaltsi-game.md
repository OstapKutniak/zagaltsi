...
**НАВМИСНА ДВОЗНАЧНІСТЬ (стрижень тону — не порушувати):** потойбіч бачить ЛИШЕ гравець, у якого від тривожності «поїхала кукуха» (психіка зламалась). Гра свідомо **НЕ підтверджує**, чи потойбіч існує насправді, чи це маячня/психоз збожеволілого персонажа. Цю неоднозначність тримаємо — вона й є фолк-горор («не ясно, де межа марення й мани»). → **Будь-яка механіка (утримання, потойбіч, знахідки) не має руйнувати цю двозначність** — напр. НЕ робити потойбіч «об'єктивно» підтвердженим ззовні (реальна фаза місяця, чужі гравці як свідки тощо роблять його «справжнім» → небажано).
...
### Петля утримання — хата + зачіпки + вечірнє полювання (напрям Фази 1, 2026-07)

Гачок утримання — **полювання на нечисть** (розвиток азарт-бази «Велика дичина»), а хата — підготовчий якір, НЕ головна тяга (щоб не з'їхати в чорну роботу). **Три якорі за добу:**

1. **Хата (раз на пару годин).** Домашня сторінка з будинком — хаб підготовки: збираєш **обереги**, **трави** тощо, підтримуєш **багаття**, керуєш/відновлюєш станом тривожності. Готує до вечірнього полювання.
2. **Зачіпки (пару разів на день).** Село/НПС підкидають натяки, де шукати нечисть. Збираєш і зважуєш: **чи сходяться зачіпки** (реальний слід) — чи це **брєд селян + когнітивне викривлення**?
3. **Вечірнє полювання (симульована північ ≈ 21:00).** Опівночі реально грають одиниці → **о 21:00 симулюємо північ**: відкривається полювання на ціль, яку наводили зачіпки.

**Механіка ВІРИ / двозначності (ключове — це стрижень, не додаток):**
- **Нормальна тривожність:** мисливські цілі мають приземлене пояснення — здоровенний **ведмідь**, схожий на монстра; **дід із палицею**, схожий на лісовика; розстроєна **вдова** замість полудениці. Ти вбиваєш… когось. Нечисть це чи людина — не ясно.
- **Висока тривожність ОБМЕЖУЄ спілкування з НПС** — буквально не можеш брати квести / збирати зачіпки в селі (люди для тебе обертаються на нечисть, з ними не договоришся).
- **Повна тривожність:** саме тоді можеш **дійсно знайти те, що шукають селяни** — справжню нечисть під приземленою маскою.
- **Наскрізь усю гру лишається неясно, чи нечисть існує.** «Віриш ти в неї» = чи склалась мозаїка зачіпок, чи це самонавіювання. НЕ підтверджувати остаточно.

**Заміна «контрактів»:** замість абстрактних контрактів — ці конкретні неоднозначні полювання (ведмідь / дід / вдова…), кожне з приземленою і потойбічною читкою водночас.
...
## ЛОББІ / ХАТИНА + РЕДАКТОР МЕНЮ — план (2026-07)

Головне меню = «лоббі» = **хатина з багаттям** (`MenuScene`, фон `public/menu/home.png` 20:9).
Заголовок ХОРУГВА зверху; кнопки ліворуч: **Мандри** (подорожі → згодом глобальна карта,
поки просто запускає гру соло) / **Завдання** / **Прогрес** / **Інвентар**. Наразі
захардкоджено в `MenuScene`; шрифт тимчасово Georgia small-caps (треба кастомний — чекаємо файл).

### Задум Остапа (напрям, ще не зроблено)
- **Персонаж сидить перед вогнищем** — підтягується з «Прогресу» (той самий кутаут-персонаж
  гравця, `zag_game_char`), у позі «сидить». Як він вдягнений — такий і сидить.
- **Шмотки/екіпіровка:** Остап сам підганяє арт шмоток, щоб «сідали» на слоти персонажа й
  анімувались (рігова система слотів). **Задача Claude:** в **Інвентарі** можна вдягатись
  (вмикати слоти-шмотки); **Прогрес** памʼятає вдягнене; персонаж перед вогнищем відображає це.
- **Вогнище:** анімувати (полумʼя) + анімація підкидання дров + кілька рівнів яскравості
  (скільки світить). Освітленням/атмосферою Остап керуватиме сам (як у редакторі рівнів).
- **Стан тривожності кімнати:** вигляд хатини змінюється при високій тривожності (можливо
  забивається рандомними ассетами) — прив'язка до потойбіч/двозначності.

### Згин хребта (spine bend) — ГОТОВО в грі, TODO в рігу (2026-07)
Персонаж сидить у лоббі перед вогнищем (`MenuScene` тягне героя з `char-library.json`,
поза `sit`). Для нормальної зігнутої спини додано **згин хребта**:
- **`CutoutCharacter.ts` (гра/лоббі) — ГОТОВО:** якщо в торса є розріз (`cut`), торс ділиться
  на **таз** (під розрізом, лишається на корені) + **грудину** (над розрізом, гнеться навколо
  суглоба на `animBend('torso')`). Шия/руки (`SPINE_CHILDREN`) і голова (через шию) **їдуть
  за грудиною** (`spine` обчислюється в `tick()` до рендеру дітей, wof обертає їх навколо суглоба).
  Це ІНВЕРСНО до кінцівок (там гнеться нижня частина). `sit` горбиться через `SIT.spine` (не через
  жорсткий `SIT.torso`). Знак `SIT.spine` додатний = вперед (перевірено).
- **Лоббі:** тимчасово `doc.slots.torso.cut = 0.5` в `MenuScene.seatCharacter` (бо в реального
  Остапа `cut≈0.082` під шиєю — тонкий сливер). ПРИБРАТИ, коли Остап виставить свій розріз у рігу.
- **`rig/main.ts` (редактор) — TODO:** його ланцюговий Canvas2D-рендер (`drawImageAt`, `worldOf`)
  ще НЕ знає про інверсний торс-спайн → у рігу `sit` показує жорсткий нахил. Треба продублювати:
  (1) `worldOf` — шия/arm_* обертаються навколо суглоба хребта; (2) `drawImageAt` — торс інверсно
...
### Перехідні анімації sit↔idle + ріалтайм-морф меню (2026-07)

**Анімації переходу `sit_down` / `stand_up`** (у `CutoutCharacter.ts` і дзеркально в
`rig/main.ts`): плавний блендінг між позами `idle` і `sit`. `TRANS_DUR = 0.7` c;
`transK(name, t)` дає коефіцієнт зі smoothstep (`p*p*(3-2p)`), `sit_down` = 0→1 у бік sit,
`stand_up` = 1→0; `animRoot`/`animOff`/`animBend` при цих іменах лерплять між idle- і
sit-результатами (`lerpN`). Додані в **BUILTIN** (гра + ріг) і в `PROC_PERIOD` рігу, тож
зʼявляються в переліку анімацій тулзи.

**Ріалтайм-морф Житло↔Інвентар (БЕЗ зміни сцени)** — за задумом Остапа: сцена не
рестартує, фон/вогонь/шрифти лишаються на місці, тільки персонаж встає/сідає, а верхній
напис плавно перетворюється. `MenuScene.morphTo(page)`: (1) tween alpha заголовка 0 →
підміна тексту → 1; (2) `char.setAnim('stand_up'|'sit_down')` + tween `charHolder` між
сидячою (x=C.x, y=C.y) і стоячою (x=C.x+55, ступні на 522) позицією за `TRANS_DUR`;
`delayedCall` ставить фінальну анімацію (`idle`/`sit`); (3) будує/знищує панель інвентаря.
Прапорець `morphing` блокує повторні кліки під час переходу.
**Спільний `src/scenes/invPanel.ts`** — `buildInvPanel(scene, offX, offY, getChar)` +
`applyEquipTo(char, equip)`: та сама UI-панель (5 слотів-шмоток праворуч + стрічка предметів
знизу) використовується І морфом у MenuScene, І окремою `InventoryScene` (для deep-link
`?startapp=inventar`). InventoryScene тепер лише тонкий сабклас MenuScene (charSetup=idle
праворуч, onCharReady вдягає збережене, afterBuild будує панель).
**TODO:** поширити морф на інші сторінки (Квести/Досягнення/Мандри/Хоругва) — Остап:
«і потім так само зробимо з іншими сторінками». Зараз морфиться лише Житло↔Інвентар,
решта кнопок — `scene.start`.
...
Додано кнопки «Скопіювати запрошення» (прямий deep-link `t.me/<bot>/<app>?startapp=kh_<id>`,
працює навіть якщо друг ніколи не відкривав бота — `BOT_USERNAME`/`APP_SHORT` у khorugva.ts),
«Покинути» (`leaveKhorugva` → member=null у Firebase + забути LS), лічильник «Побратимів: N/5».
⚠️ **ФІКС «приєднання не працює» (2026-07):** насправді паті працювало через Firebase —
баг був чисто в рендері: `renderSlots` малював просто в сцену, і при кожному watch-оновленні
СТАРІ портрети лишалися ПІД новими (виглядало як зависання). Тепер увесь вміст слотів — у
`slotLayer` (container), який `destroy(true)` на кожне оновлення; `drawKhSlot`/`khPortraits.ts`
приймають опційний контейнер-власник `into`. Ніякого додаткового воркера не треба.
...
### Редактор Історії (2026-07) — колишня вкладка «Редактор Меню»
Вкладка studio перейменована на **«Редактор Історії»**; у правій панелі перемикач
4 розділів (src/story/editor.ts):
- **Меню** — редактор сторінок меню (як був, src/menu/editor.ts);
- **Квести** — список квестів {title, text, cat: main|side} (src/story/quests.ts;
  IDB `zag_quests` ↔ публікація `studio-data/quests.json`, LWW). Гра: QuestsScene —
  дошка з нотатками-квестами (головні з ★ і першими), тап → повний текст оверлеєм.
  Контейнер-нотатка: хіт-зона ЯВНИМ Rectangle(-w/2,-h/2) — setSize дає зміщену зону;
- **Бот** — редактор bot-config.json (initBotEditor(container));
- **Діалоги** — вибір персонажа (charlib) + mountDialogEditor. Перенесено з поведінки
  ворогів; у рігу вкладка «Діалоги» ЛИШИЛАСЬ як контекстне прев'ю при поведінці НПС
  (не видалено навмисно — прибрати, якщо Остап скаже).
TODO квестів далі: статус «виконано», прив'язка до контрактів/полювань, нагорода.

### Енкаунтери + нагадування (2026-07) — петля утримання ЖИВА
**Флоу:** бот-cron (щогодини, шанс `encounter.chance` на гравця) шле «У вашій локації
хтось є…» + [Перевірити] → deep-link `enc` → гра відкриває ПОТОЧНУ локацію мандрівника
(loadTravel; фолбек — перша локація по всіх світах) → `LocationScene.spawnEncounter`:
НПС-видавець квесту (CutoutCharacter, idle, передній план) + його діалог-дерево
(`src/dialogPlay.ts` — програвач DialogDoc, кулька+варіанти) → кінцівка **positive**
(або квест без діалогу) → `acceptQuest` → нотатка на дошці «Завдання» → воркер-cron
шле нагадування раз на `remind.hours` (5) год з [До завдань].
**Дані:** квест має `giver` (charId) + `dialogId` (селекти в панелі «Квести»);
взяті квести — localStorage `zag_taken_quests` + Firebase `player_quests/{playerId}`
(tg user id == chat_id привату — воркер шле напряму). Дошка показує giver-квести
лише ПІСЛЯ взяття; безгіверні — одразу. Тексти/шанс/години — у bot-config (панель «Бот»).
**Тест руками:** `GET <воркер>/test-encounter?nick=<нік>` — миттєвий енкаунтер собі.
**ВАЖЛИВИЙ ФІКС (`src/store.ts`):** IDB-з'єднання тепер ОДНЕ кешоване на сторінку.
Раніше кожен idbGet/idbSet відкривав нове й не закривав → сотні висячих конектів →
`indexedDB.open()` зависав назавжди (мовчки не вантажились квести/діалоги/локації).
НЕ повертати по-з'єднанню-на-виклик.
...
- **Дані:** `MenuDoc` в IndexedDB (`zag_menu`), синк LWW як інші доки. Список **сторінок**;
  кожна сторінка = фон + елементи.
- **Елементи:** текст/кнопка `{text, font, size, color, hoverColor, x, y, align, action}`
  (action: `page`/`scene`/`game`/`url` = навігація-«гіперпосилання»); зображення; **слот
  персонажа** (спецелемент — позиція/масштаб, заповнюється рантайм із `zag_game_char`+екіп);
  анімовані елементи (вогнище). Опційно — атмосфера сторінки (реюз системи рівнів).
- **Рушій:** та сама drag/G/R/S/пан/зум канва, що level/world/location редактори (реюз).
- **Гра:** `MenuScene` стає **data-driven** — читає `MenuDoc` і рендерить сторінки узагальнено;
  кнопки-дії керують навігацією. Це прибирає хардкод, і Остап сам міняє шрифти/розкладку/персонажа.
- **Порядок збірки (пропозиція):** (1) data-driven меню + мінімальний редактор елементів →
  (2) слот персонажа перед вогнищем (поза «сидить») → (3) вогнище-анімація + рівні світла →
  (4) Інвентар: вдягання + Прогрес памʼятає екіп → (5) стан тривожності кімнати → (6) атмосфера сторінок.

## Bretwalda — вкладиші українською (bretwalda.html)

Побічна тулза (не про гру Хоругва): генератор українських перекладів-вкладишів
для настільної гри Sovereign: Bretwalda (PHALANX) + доповнення. Вкладиш — смужка,
що закриває нижню текстову частину карти в протекторі; арт і англійська назва
лишаються видимі.

- Фон: чистий 300dpi шар без тексту, витягнутий з видавничого PDF (Danelag D33
  bleeds) — `public/bretwalda/bg-clean.png`. Значок Danelag вирізаний окремо
  (`icon-danelag.png`), місце під значок залатане пергаментом.
- Шрифт: Monomakh Unicode (кириличний півустав, SIL OFL) — історичний аналог
  інсулярного унціалу оригінальних шрифтів гри (BRETWALDABIG1/SMALL, кирилиці
  не мають). Карта 57×89 мм (з блідів PDF: 63×95 мінус 3 мм).
- Тулза: список карток (localStorage + експорт/імпорт JSON), редагований текст
  (рядок-тригер + основний, токени {монета} {кубик} {меч} {товар} {рух}), пресети
  доповнень (перефарбування рамки по масці за яскравістю, свій значок з фото
  користувача), PDF-експорт аркушів A4 у точному масштабі з мітками порізки і
  лінійкою 50 мм (jsPDF). Код: `src/bretwalda/main.ts`.
- Два розділи колоди: **карти Влади** (вертикальні, рідний фон bg-clean.png) і
  **карти Хроніки** (горизонтальні, вкладиш на всю ширину карти від верху стрічки
  назви; назва українською + текст умови). Словник термінів перекладу:
  [bretwalda-glossary.md](bretwalda-glossary.md) — нові картки перекладати лише за ним.
- ⚠️ **TODO: фон карт Хроніки** — зараз тимчасовий згенерований пергамент
  (`drawChronicleStrip`), рідного шаблону для Хронік ще нема. Коли Остап знайде
  скан/шаблон — підставити фон під усі картки Хронік одним махом (як bg-clean.png
  для карт Влади). Точні розміри карт Хроніки теж уточнити (зараз 88×~57 мм,
  смужка 30 мм — приблизно).
