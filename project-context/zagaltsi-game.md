...
- Тулза: список карток (localStorage + експорт/імпорт JSON), редагований текст
  (рядок-тригер + основний, токени {монета} {кубик} {меч}), пресети доповнень
  (перефарбування рамки по масці за яскравістю, свій значок з фото користувача),
  PDF-експорт аркушів A4 у точному масштабі з мітками порізки і лінійкою 50 мм
  (jsPDF). Код: `src/bretwalda/main.ts`.
